...
exports.requireSignin = (req, res, next) => {
  const token = req.headers.authorization.split(" ")[1]; //in postman under headers section we created the authorization parameter which had values "Bearer token".On using split function it places then in an array in such a way ["Bearer","token"] and we select the token from it
  const user = jwt.verify(token, process.env.JWT_SECRET); //jwt.verify checks the token generated when the user is created with the token that would be generated by the secret code.
  req.user = user;
  next();
};
